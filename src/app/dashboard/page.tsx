  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
        {rows.map(row => {
          const newestRun = row.runs[0]
          const lastActive = row.runs.find(r => r.status !== 'not_run')
          const status = lastActive?.status ?? 'not_run'
          const color = STATUS_COLOR_MAP[status] ?? 'var(--text-faint)'
          const d = newestRun ? new Date(newestRun.started_at) : null
          const dateLabel = d ? `${d.getMonth()+1}/${d.getDate()} ${String(d.getHours()).padStart(2,'0')}h` : '—'
          const stale = newestRun && lastActive && newestRun.run_id !== lastActive.run_id
          ...
          return (
            ...
            >
              <span style={{ width: 8, height: 8, borderRadius: 2, background: color, opacity: stale ? 0.45 : 0.88, flexShrink: 0 }} />
              ...
              <span style={{ fontSize: 10, color: stale ? 'var(--text-faint)' : color, fontWeight: 500, flexShrink: 0 }}>
...
export default function Dashboard() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const moduleUrl = `/api/test-stats?type=module-stats`
